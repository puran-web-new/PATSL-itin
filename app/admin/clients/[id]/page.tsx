...
export default function ClientDetailPage() {
  ...
  return (
    ...
    >
      <Link href="/admin/clients" className="mb-4 inline-block text-xs font-semibold text-slate-500 hover:text-brand-700">&larr; All clients</Link>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5">
              ...
              <div className="space-y-1">
                <Link href={`/admin/applications/${application.id}`} className="block rounded-lg bg-brand-600 px-3 py-2.5 text-center text-xs font-semibold text-white hover:bg-brand-500">
                ...
                {invoices.map((inv) => (
                  <div key={inv.id} className="rounded-lg border border-slate-100 px-3 py-2 text-xs">
                    ...
                    {inv.square_payment_link && (
                      <a href={inv.square_payment_link} target="_blank" rel="noreferrer" className="text-[10.5px] font-semibold text-brand-700 hover:underline">View payment link</a>
                    )}
                  </div>
                ))}
              </div>
